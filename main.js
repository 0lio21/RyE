...
; (function (document, window, index) {
  ...
  Array.prototype.forEach.call(inputs, function (input) {
    ...
    input.addEventListener('change', function (e) {
      ...
      // aca valida el tamanio del archivo
      const TamMax = 30 * 1024 * 1024; // 30 MB en bytes
      let validFiles = true; //esta variable desp se usa para ver si todos los archivos seleccionados cumplen con el requisito de tamanio.
      for (let i = 0; i < this.files.length; i++) {
        if (this.files[i].size > TamMax) {
          alert('El archivo ' + this.files[i].name + ' es demasiado grande. El tamaño máximo permitido es de 30 MB.');
          this.value = '';
      ...
      // muestra el boton enviar si es valido el correo y el tamanio de los archivos
      if (validFiles && this.files.length > 0) {
        document.getElementById('enviarBoton').style.display = 'block';
      } else {
        document.getElementById('enviarBoton').style.display = 'none';
      }
